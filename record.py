...
def handler_date_time(message, user_data):
    try:
        ...
        bot.send_message(message.chat.id, "Спасибо", reply_markup=hide_markup)
    except KeyError:
        bot.send_message(message.chat.id, "Произошла ошибка. Пожалуйста, выберите дату заново.")
...
def generate_all_slots() -> list:
    ...
    """
    time_zone = pytz.timezone('Asia/Yakutsk')
...
def send_file_boss(message):
    user_id = os.getenv('user_file')
